# Fall Detection — Docker Deployment Guide (Hetzner / MCS)
...
|---------|------|---------|---------|
| inference-server | 8001 | Fall detection API (mobile app calls this) | Yes — put behind reverse proxy |
...
| mqtt | 1883/8883 | MQTT broker (mobile app publishes fall alerts) | Yes — port 1883 or 8883 |
...
Open `deploy/.env` and fill in every `CHANGE_ME` value:
...
`minio-setup`) will show `exited (0)` — that is correct.
...
| ML Dashboard | http://YOUR_SERVER:8004/ | none (add auth — see below) |
...
docker compose -f deploy/docker-compose.yml --env-file deploy/.env build inference-server
docker compose -f deploy/docker-compose.yml --env-file deploy/.env up -d --no-deps inference-server
```
...
docker compose -f deploy/docker-compose.yml logs -f inference-server
...
docker compose -f deploy/docker-compose.yml restart fall-dashboard

# Run alembic migration manually (e.g. after an upgrade)
docker compose -f deploy/docker-compose.yml run --rm db-migrate
```

## Shorthand (run from inside deploy/)

If you cd into the `deploy/` directory, Docker Compose will automatically pick
...
**TLS is required** — the mobile app will refuse plain HTTP connections.
Use Let's Encrypt (free, auto-renews every 90 days) via Certbot + Nginx:

**Step 1 — DNS:** Ask the MCS domain admin to create an A-record pointing
a subdomain to this Hetzner server's IP address.
Example: `fall-api.mcs-labs.de` → `<hetzner-server-ip>`
...
**Step 4 — Add reverse proxy rules** to `/etc/nginx/sites-available/default`:
```nginx
server {
    listen 443 ssl;
...
By default there is no authentication — set `MQTT_USERNAME` / `MQTT_PASSWORD` in
`.env` and follow the steps in `deploy/mosquitto.conf` to enable password auth.
...
```bash
docker compose -f deploy/docker-compose.yml logs db-migrate
```

**inference-server health check failing:**
Model file may be missing from `model/` directory. Check:
```bash
docker compose -f deploy/docker-compose.yml logs inference-server
```

**MLflow can't connect to MinIO:**
Check minio-setup completed successfully:
```bash
docker compose -f deploy/docker-compose.yml logs minio-setup
```
...
```bash
docker compose -f deploy/docker-compose.yml exec fall-dashboard \
