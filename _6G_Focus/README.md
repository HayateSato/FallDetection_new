...
| File | Purpose |
|------|---------|
| `README.md` | This file — deployment guide, start here |
| `system_overview.md` | How the three-party system fits together (diagram) |
| `api_reference.md` | Fall dashboard HTTP API and SSE event contract |
| `helm/` | Helm chart — `values.yaml` is the only file you need to edit |
...
**Background:** The original plan was for FOCUS to host the full stack on your K3s cluster. Due to hardware constraints, you requested that MCS take over hosting entirely. We are happy to host the compute-heavy parts of the system (inference, model management, monitoring), however we strongly recommend that FOCUS continues to host the two services in this package. The reason is **data privacy**: these two services are the only ones that handle patient identifiers and fall history. Keeping them within your own infrastructure means patient data never leaves your premises, which is important both for regulatory compliance and for patient trust. The two services are intentionally lightweight — they require minimal CPU and memory and are fully manageable with standard Helm commands.
...
- [ ] **1. Create two subdomains** 
  ...
  2) subdomain for the fall dashboard (e.g. `fall.focus-hospital.de`) — caregiver needs to open this to check real time fall alert or fall history - please share this when they need to monitor the patients. 
- [ ] **2. Pull docker images** — fall-dashboard Docker image is already pushed to `registry.smarko-health.de`.
- [ ] **3. Follow and complete 5 steps from deployment guide below** 
  1)  Firewall setting (skip if already allowed)
  2)  Cluster set up (create namespace + pull secret + Traefik MQTT entrypoint)
  3)  Update helm/values.yaml (update all `CHANGE_ME` fields + name of the Let's Encrypt resolver)
...
- [ ] **4. Share your subdomains with Isa** — he needs them for the mobile app config:
  ...
  - MQTT username + password ([optional] if you enabled broker auth in `helm/values.yaml`)
...
### 2) `fall-dashboard` — caregiver alert dashboard
...
- Helm chart (`helm/` folder) and `helm/values.yaml`; the Docker image is already in the registry
...
### STEP 3: Fill in helm/values.yaml

Open `helm/values.yaml`. There are **3 fields you must fill in**, 4 optional fields, and the rest is pre-filled — do not change those.
...
(Run below from the root of this package as working directory)

```bash
# First install — helm auto-loads helm/values.yaml
helm install caregiver helm/ --namespace fall-dashboard

# Future updates (e.g. values change or image update)
helm upgrade caregiver helm/ --namespace fall-dashboard
```

> `helm upgrade` is required whenever `helm/values.yaml` changes — K3s does not
...
|------|-----------------|----------------------|
| MQTT broker domain | `mosquitto.ingress.host` in `helm/values.yaml` | Mobile app connects as `wss://<domain>:443` to publish fall alerts |
