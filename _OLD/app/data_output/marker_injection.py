"""
manual truth Writer for InfluxDB

Writes manual truth annotations directly to InfluxDB as a separate field.
When user presses the manual truth button, value '1' is written at that exact timestamp.
"""
...
def write_manual_truth_marker(value: int, measurement: str = "SMART_DATA") -> bool:
    """
    Write a manual truth marker to InfluxDB at the current timestamp.
    Uses SMART_DATA measurement to appear at same level as sensor data.

    Args:
        value: manual truth value (1 = fall event marker)
    ...
    try:
        ...
        # This ensures manual_truth appears as a field alongside bosch_acc_x, etc.
        ...
        logger.info(f"manual truth marker written: value={value} at {timestamp.isoformat()}")
        return True

    except Exception as e:
        logger.error(f"Failed to write manual truth marker: {e}", exc_info=True)
        return False
...
def write_manual_truth_event(event_type: str, value: int = 1) -> bool:
    """
    Write a manual truth event marker to InfluxDB.
    ...
    try:
        ...
        logger.info(f"manual truth event written: {event_type}={value} at {timestamp.isoformat()}")
        return True

    except Exception as e:
        logger.error(f"Failed to write manual truth event: {e}", exc_info=True)
        return False
    # Note: Don't close the client - it's a singleton that will be reused
