...
- [Read ACC from SmarKo wearable via BLE](#1-sensor-data-reading)
- [Write `SMART_DATA` to FOCUS InfluxDB continuously](#1-sensor-data-reading)
- [Query `/model/info` on startup](#2-fall-detection-calling-inference-server)
- [POST `/predict` to MCS inference server](#2-fall-detection-calling-inference-server)
- [Publish `fall/possible/<pid>` immediately on fall detected](#3-immediate-mqtt-publish-before-patient-sees-popup)
- [Show patient confirmation popup (10s per question)](#4-patient-confirmation-popup)
- [POST `/inference/{obs_id}/confirm` to MCS inference server](#5-after-patient-confirmation-or-timeout)
- [Publish `fall/alert/<pid>` via MQTT WebSocket](#5-after-patient-confirmation-or-timeout)
- [Write `fall_events` point to FOCUS InfluxDB](#5-after-patient-confirmation-or-timeout)
- [Record `predicted_at`, `confirmed_at`, `help_requested_at` timestamps](#6-timestamp-logging)
