...
class StreamingPaperBarometerProcessor:
    """
    Real-time streaming processor implementing the paper's approach.

    Maintains internal buffers for slope-limit and moving average filters,
    suitable for sample-by-sample processing in embedded systems.
    """
    ...
    def __init__(self, config: PaperBarometerConfig):
        ...
        # Internal state
        self.last_slope_output = None
        self.ma_buffer = []
        self.is_initialized = False

    def process_sample(self, pressure: float) -> float:
        """
        ...
        """
        # Slope-limit filter
        if self.last_slope_output is None:
            slope_output = pressure
        else:
            diff = pressure - self.last_slope_output
            if diff > self.slope_limit:
                slope_output = self.last_slope_output + self.slope_limit
            elif diff < -self.slope_limit:
                slope_output = self.last_slope_output - self.slope_limit
            else:
                slope_output = pressure

        self.last_slope_output = slope_output

        # Moving average filter
        self.ma_buffer.append(slope_output)
        if len(self.ma_buffer) > self.ma_window_samples:
            self.ma_buffer.pop(0)

        filtered_output = np.mean(self.ma_buffer)

        if not self.is_initialized and len(self.ma_buffer) >= self.ma_window_samples:
    ...
    def is_ready(self) -> bool:
        """Check if processor has filled its buffers."""
